...
/** Path to the local ng-extra. By default based on the location of this script. */
 ...
 * location.
 */
const presubmitScript = `${process.env.NGE_PRESUBMIT_DIR}/ng-extra-presubmit.sh`;
...
if (explicitPullRequests.length) {
  ...
  github.search.issues({
    per_page: 100,
    q: 'repo:gnucoop/ng-extra is:open type:pr label:"pr: merge ready" -label:"pr: merge safe"',
  }, (error, response) => {
    if (response) {
