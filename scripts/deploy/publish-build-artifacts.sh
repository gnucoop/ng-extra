...
if [ -z ${NG_EXTRA_BUILDS_TOKEN} ]; then
  echo "Error: No access token for GitHub could be found." \
       "Please set the environment variable 'NG_EXTRA_BUILDS_TOKEN'."
  exit 1
fi

# Ng-Extra packages that need to published.
PACKAGES=(ng-extra mat-extra ion-extra)
REPOSITORIES=(ng-extra-builds mat-extra-builds ion-extra-builds)
...
publishPackage() {
  ...
  echo "sed -i \"s/${buildVersion}/${commitTag}/g\" \$(find . -type f -not -path '*\/.*')"
  ...
  echo "https://${NG_EXTRA_BUILDS_TOKEN}:@github.com" > .git/credentials
