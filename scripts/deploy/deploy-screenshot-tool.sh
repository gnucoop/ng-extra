...
if [ -z ${NG_EXTRA_SCREENSHOT_FIREBASE_DEPLOY_TOKEN} ]; then
  echo "Error: No access token for firebase specified." \
       "Please set the environment variable 'NG_EXTRA_SCREENSHOT_FIREBASE_DEPLOY_TOKEN'."
  exit 1
fi
...
${firebaseBin} deploy --token ${NG_EXTRA_SCREENSHOT_FIREBASE_DEPLOY_TOKEN} --non-interactive \
  --project ng-extra-screenshots
