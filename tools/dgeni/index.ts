...
// Package definition for ng-extra api docs. This only *defines* the package- it does not yet
...
/** List of Ng-Extra packages that need to be documented. */
const ngExtraPackages = globSync(path.join(sourceDir, 'ng-extra', '*/'))
  .filter(packagePath => !packagePath.endsWith('testing/'))
  .map(packagePath => path.basename(packagePath));

/** List of Mat-Extra packages that need to be documented. */
const matExtraPackages = globSync(path.join(sourceDir, 'mat-extra', '*/'))
  .map(packagePath => path.basename(packagePath));

/** List of Mat-Extra packages that need to be documented. */
const ionExtraPackages = globSync(path.join(sourceDir, 'ion-extra', '*/'))
  .map(packagePath => path.basename(packagePath));

export const apiDocsPackage = new Package('ng-extra-api-docs', dgeniPackageDeps);
...
apiDocsPackage.config((readTypeScriptModules: ReadTypeScriptModules, tsParser: TsParser) => {
  ...
  ngExtraPackages.forEach(packageName => {
    typescriptPathMap[`@gnucoop/ng-extra/${packageName}`] = [`./ng-extra/${packageName}/index.ts`];
  });

  matExtraPackages.forEach(packageName => {
    typescriptPathMap[`@gnucoop/mat-extra/${packageName}`] =
      [`./mat-extra/${packageName}/index.ts`];
  });

  ionExtraPackages.forEach(packageName => {
    typescriptPathMap[`@gnucoop/ion-extra/${packageName}`] =
      [`./ion-extra/${packageName}/index.ts`];
  });
  ...
  readTypeScriptModules.sourceFiles = [
    ...ngExtraPackages.map(packageName => `./ng-extra/${packageName}/index.ts`),
    ...matExtraPackages.map(packageName => `./mat-extra/${packageName}/index.ts`),
    ...ionExtraPackages.map(packageName => `./ion-extra/${packageName}/index.ts`)
  ];
});
