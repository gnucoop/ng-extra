...
export class PayloadChart {
  ...
  private createChartResults(data: PayloadResult[]) {
    ...
    const ngExtraChartItems: NgxChartItem[] = [];
    const matExtraChartItems: NgxChartItem[] = [];
    const ionExtraChartItems: NgxChartItem[] = [];

    // Walk through every result entry and create a NgxChart item that can be rendered inside of
    // the linear chart.
    data.forEach(result => {
      if (
        !result.ng_extra_fesm_2015 || !result.mat_extra_fesm_2015 || !result.ion_extra_fesm_2015
      ) {
        return;
      }
      ...
      ngExtraChartItems.push({ name: date, value: result.ng_extra_fesm_2015 });
      matExtraChartItems.push({ name: date, value: result.mat_extra_fesm_2015 });
      ionExtraChartItems.push({ name: date, value: result.ion_extra_fesm_2015 });
    });

    return [
      { name: 'Ng-Extra', series: ngExtraChartItems },
      { name: 'Mat-Extra', series: matExtraChartItems },
      { name: 'Ion-Extra', series: ionExtraChartItems }
    ];
  }
