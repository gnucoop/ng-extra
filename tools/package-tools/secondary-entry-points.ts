 ...
 *
 * @param pkg The package for which to get entry points, e.g., 'ng-extra'.
 * @returns An array of secondary entry-points names, e.g., ['geolocation', ...]
 */
export function getSecondaryEntryPointsForPackage(pkg: BuildPackage) {
  ...
  // E.g., extract "geolocation" from "from '@gnucoop/ng-extra/geolocation';".
...
function buildPackageImportStatementFindCommand(searchDirectory: string, packageName: string) {
  if (platform() === 'win32') {
    return {
      binary: 'findstr',
      args: ['/r', `from.'@gnucoop/${packageName}/.*'`, `${searchDirectory}\\*.ts`]
    };
  } else {
    return {
      binary: 'grep',
      args: ['-Eroh', '--include', '*.ts', `from '@gnucoop/${packageName}/.+';`, searchDirectory]
    };
  }
}
