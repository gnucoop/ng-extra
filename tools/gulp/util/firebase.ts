...
const dashboardDatabaseUrl = 'https://ng-extra-board.firebaseio.com';
...
export function openFirebaseDashboardApp() {
  ...
  return firebaseAdmin.initializeApp({
    ...
    credential: firebaseAdmin.credential.cert({
      project_id: 'ng-extra-board',
      client_email: 'ng-extra-board@appspot.gserviceaccount.com',
      // In Travis CI the private key will be incorrect because the line-breaks are escaped.
      // The line-breaks need to persist in the service account private key.
      private_key: decode(process.env['NG_EXTRA_BOARD_FIREBASE_SERVICE_KEY']!)
    }),
  });
}
...
export function openScreenshotsBucket() {
  ...
  return gcs.bucket('ng-extra-screenshots.appspot.com');
}
