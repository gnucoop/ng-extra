...
import {buildConfig, sequenceTask, watchFiles} from 'ng-extra-build-tools';
...
task('e2e-app:build', sequenceTask(
  'clean',
  [
    'ng-extra:build-release',
    'mat-extra:build-release',
    'ion-extra:build-release',
    'mat-extra-examples:build-release',
    'ion-extra-examples:build-release'
  ],
 ...
 */
task('serve:e2eapp:watch', ['serve:e2eapp', 'mat-extra:watch', ':watch:e2eapp']);

// As a workaround for https://github.com/angular/angular/issues/12249, we need to
// copy the Ng-Extra, Mat-Extra and Ion-Extra ESM output inside of the demo-app output.
task('e2e-app:copy-release', () => {
  copySync(join(releasesDir, 'ng-extra'), join(outDir, 'ng-extra'));
  copySync(join(releasesDir, 'mat-extra'), join(outDir, 'mat-extra'));
  copySync(join(releasesDir, 'ion-extra'), join(outDir, 'ion-extra'));
  copySync(join(releasesDir, 'mat-extra-examples'), join(outDir, 'mat-extra-examples'));
  copySync(join(releasesDir, 'ion-extra-examples'), join(outDir, 'ion-extra-examples'));
});
