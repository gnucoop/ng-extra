...
const examplesPaths = [
  path.join(packagesDir, 'mat-extra-examples'),
  path.join(packagesDir, 'ion-extra-examples')
];

/** Output path of the module that is being created */
const outputModuleFilenames = examplesPaths.map((examplesPath) => {
  return path.join(examplesPath, 'example-module.ts');
});

/** Build ES module import statements for the examples. */
function buildImportsTemplate(examplesIdx: number, metadata: ExampleMetadata): string {
  ...
  const relativeSrcPath = path
    .relative(examplesPaths[examplesIdx], metadata.sourcePath)
    .replace(/\\/g, '/')
    .replace('.ts', '');
 ...
 */
function generateExampleNgModule(
  examplesIdx: number, extractedMetadata: ExampleMetadata[]
): string {
  return `
/* tslint:disable */
...
${extractedMetadata.map(r => buildImportsTemplate(examplesIdx, r)).join('').trim()}
...
task('build-examples-module', () => {
  examplesPaths.forEach((examplesPath, idx) => {
    ...
    const generatedModuleFile = generateExampleNgModule(idx, results);
    fs.writeFileSync(outputModuleFilenames[idx], generatedModuleFile);
  });
});
