...
function checkEs2015ReleaseBundle(packageName: string, bundlePath: string): string[] {
  ...
  if (packageName === 'mat-extra' || packageName == 'ion-extra') {
    failures = failures.concat(checkNgExtraThemedPackage(packageName));
  }

  return failures;
}

/** Function that includes special checks for the Mat-Extra package. */
function checkNgExtraThemedPackage(packageName: string): string[] {
  ...
  if (glob('*.css', {cwd: prebuiltThemesPath}).length === 0) {
    failures.push('Prebuilt themes are not present in the Mat-Extra release output.');
  }

  if (!existsSync(themingFilePath)) {
    failures.push('The theming SCSS file is not present in the Mat-Extra release output.');
  }
