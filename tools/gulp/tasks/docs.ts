...
import {buildConfig, sequenceTask} from 'ng-extra-build-tools';
...
// `ng-extra-docs-example` attribute which can be used to locate the divs and initialize the example
// viewer.
const EXAMPLE_PATTERN = /<!--\W*example\(([^)]+)\)\W*-->/g;

// Markdown files can contain links to other markdown files.
// Most of those links don't work in the Ng-Extra docs, because the paths are invalid in the
...
task('docs', sequenceTask(
  [
    'markdown-docs-mat-extra',
    'markdown-docs-ion-extra',
    'markdown-docs-ng-extra',
    'build-mat-extra-highlighted-examples',
    'build-ion-extra-highlighted-examples',
    'build-examples-module',
    'api-docs',
    'copy-mat-extra-stackblitz-examples',
    'copy-ion-extra-stackblitz-examples'
  ],
  'minify-html-files'
));

/** Generates html files from the markdown overviews and guides for Mat-Extra. */
task('markdown-docs-mat-extra', () => {
  ...
  return src(['src/mat-extra/**/!(README).md', 'guides/mat-extra/*.md'])
      .pipe(rename({prefix: 'mat-extra-'}))
...
/** Generates html files from the markdown overviews and guides for Ion-Extra. */
task('markdown-docs-ion-extra', () => {
  // Extend the renderer for custom heading anchor rendering
  markdown.marked.Renderer.prototype.heading = (text: string, level: number): string => {
    if (level === 3 || level === 4) {
      const escapedText = text.toLowerCase().replace(/[^\w]+/g, '-');
  ...
  return src(['src/ion-extra/**/!(README).md', 'guides/ion-extra/*.md'])
      .pipe(rename({prefix: 'ion-extra-'}))
      .pipe(markdown(markdownOptions))
      .pipe(transform(transformMarkdownFiles))
      .pipe(dom(createTagNameAliaser('docs-markdown')))
      .pipe(dest('dist/docs/markdown'));
...
// TODO(jelbourn): figure out how to avoid duplicating this task w/ mat-extra while still
// disambiguating the output.
/** Generates html files from the markdown overviews and guides for the ng-extra. */
task('markdown-docs-ng-extra', () => {
  return src(['src/ng-extra/**/!(README).md'])
      .pipe(rename({prefix: 'ng-extra-'}))
      .pipe(markdown(markdownOptions))
      .pipe(transform(transformMarkdownFiles))
      .pipe(dom(createTagNameAliaser('docs-markdown')))
      .pipe(dest('dist/docs/markdown'));
 ...
 */
task('build-mat-extra-highlighted-examples', () => {
  ...
  return src('src/mat-extra-examples/**/*.+(html|css|ts)')
      .pipe(flatten())
      .pipe(rename(renameFile))
      .pipe(highlight())
      .pipe(dest('dist/docs/examples/mat-extra'));
});
 ...
 * live examples on the docs site.
 */
task('build-ion-extra-highlighted-examples', () => {
  // rename files to fit format: [filename]-[filetype].html
  const renameFile = (filePath: any) => {
    const extension = filePath.extname.slice(1);
    filePath.basename = `${filePath.basename}-${extension}`;
  ...
  return src('src/ion-extra-examples/**/*.+(html|css|ts)')
      .pipe(flatten())
      .pipe(rename(renameFile))
      .pipe(highlight())
      .pipe(dest('dist/docs/examples/ion-extra'));
});
...
task('copy-mat-extra-stackblitz-examples', () => {
  src(path.join(packagesDir, 'mat-extra-examples', '**/*'))
      .pipe(dest(path.join(DIST_DOCS, 'stackblitz', 'mat-extra-examples')));
});

/** Copies example sources to be used as stackblitz assets for the docs site. */
task('copy-ion-extra-stackblitz-examples', () => {
  src(path.join(packagesDir, 'ion-extra-examples', '**/*'))
      .pipe(dest(path.join(DIST_DOCS, 'stackblitz', 'ion-extra-examples')));
});
...
function transformMarkdownFiles(buffer: Buffer, file: any): string {
  ...
  content = content.replace(EXAMPLE_PATTERN, (_match: string, name: string) =>
    `<div ng-extra-docs-example="${name}"></div>`
  );
